...
import gr.aueb.cf.schoolapp.dto.TeacherFilterDTO;
...
/**
 * Converts the TeacherInsertDTO to a Teacher entity which will be persisted in the database.
 * Converts the Teacher entity to a TeacherReadOnlyDTO for returning the response
 */
public class Mapper {
    ...
    public static List<TeacherReadOnlyDTO> teacherReadOnlyDTOs(List<Teacher> teachers) {
        return teachers.stream().map(Mapper::mapToTeacherReadOnlyDTO).collect(Collectors.toList());
    }

    public static Map<String , Object> mapToCriteria(TeacherFilterDTO filtersDTO){
